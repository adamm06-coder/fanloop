...
The same file's `gates` array is the only gate table on the page: A, B_lite,
B, C1, C2, C3, D, E, F. Each row is red, yellow, green, or unset. Week 1
marks A yellow because the local STL smoke exists and is only a visual
solid. The other rows stay unset. Green is unused. B_lite stays a DEMO
label. The words for those rows live in `cases/ducted-fan-1kn/GATES.md`.
